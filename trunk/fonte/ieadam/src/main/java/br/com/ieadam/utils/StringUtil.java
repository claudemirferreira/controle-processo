...
public class StringUtil {
    ...
    /**
     * Atualiza o numero do telefone informado para o padrao (92)9999-9999.
     * Adiciona o ddd 92, caso n�o seje informado
     * @param fone
     */
    public static String formatarNumeroTelefone( String numeroFone ) {
        String ddd, pref, suf, foneNum, fone = null;
        StringBuilder sb = new StringBuilder();
        foneNum = new String( numeroFone );
        
        // limpa a string para manipular apenas numeros
        for ( int i = 0; i < foneNum.length(); i++ ) {
            if ( Character.isDigit( foneNum.charAt( i ) ) ) {
                sb.append( foneNum.charAt( i ) );
            }
        }
        fone = sb.toString();
        
        // verifica se o telefone est� sem o ddd
        if ( fone.length() == IntegerConstantUtils.OITO ) {
            // adiciona o ddd
            fone = "92".concat( fone );
        }
        
        ddd = fone.substring( 0, 2 );
        pref = fone.substring( 2, IntegerConstantUtils.SEIS );
        suf = fone.substring( IntegerConstantUtils.SEIS, IntegerConstantUtils.DEZ );
        
        fone  = String.format( "(%02d) %02d-%04d", 
                                new Integer( ddd ), 
                                new Integer( pref ), 
                                new Integer( suf ) );
        return fone;
